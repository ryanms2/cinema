...
export function ListFilms({
  ...
  useEffect(() => {
    ...
    const dateLast =
      inputDateLast === '&primary_release_date.lte=' ? '' : inputDateLast
    const dateFirst =
      inputPrimaryDateFirst === '&primary_release_date.gte='
        ? ''
        : inputPrimaryDateFirst
    ...
    if (inputPage !== 1) {
      const fetchData = async () => {
        setLoading(true)
        const newMovies = await fetchMoviesFilter(
          dateFirst,
          dateLast,
          genres,
          inputRangeValue,
      ...
      const fetchData = async () => {
        setLoading(true)
        const total = await fetchMoviesFilter(
          dateFirst,
          dateLast,
          genres,
          inputRangeValue,
