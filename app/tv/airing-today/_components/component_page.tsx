...
export default function ComponentPage() {
  ...
  const formattedDateLast = format(
    startOfMonth(currentDateLast).setUTCDate(28),
    'yyyy-MM-dd',
  )
  const [primaryDateLast, setPrimaryDateLast] =
    useState<string>(formattedDateLast)
  const currentDate = new Date()
  const formattedDateFirst = format(
    startOfMonth(currentDate).setUTCDate(28),
    'yyyy-MM-dd',
  )
  const [primaryDateFirst, setPrimaryDateFirst] =
    useState<string>(formattedDateFirst)
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      <div className="max-w-7xl mx-auto p-4">
        <h1 className="font-extrabold text-2xl">Séries emitidas hoje</h1>
                ...
                <div className="grid gap-6">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      <Button
                        className={`${selectedGenres?.includes(10759) ? 'bg-blue-500 text-white' : ''}`}
                        onClick={() => handleBadgeClick(10759)}
                      >
                        Ação e Aventura
                      </Button>
                      <Button
                      ...
                      <Button
                        className={`${selectedGenres?.includes(10751) ? 'bg-blue-500 text-white' : ''}`}
                        onClick={() => handleBadgeClick(10751)}
                      >
                        Família
                      </Button>
                      <Button
                        className={`${selectedGenres?.includes(9648) ? 'bg-blue-500 text-white' : ''}`}
                        onClick={() => handleBadgeClick(9648)}
                      >
                        Mistério
                      </Button>
                      <Button
                        className={`${selectedGenres?.includes(10764) ? 'bg-blue-500 text-white' : ''}`}
                        onClick={() => handleBadgeClick(10764)}
                      >
                        Reality
                      </Button>
                      <Button
                        className={`${selectedGenres?.includes(80) ? 'bg-blue-500 text-white' : ''}`}
                        onClick={() => handleBadgeClick(80)}
                      >
                        Crime
                      </Button>
                      <Button
                        className={`${selectedGenres?.includes(16) ? 'bg-blue-500 text-white' : ''}`}
