    ...
    <div className="bg-gray-50 min-h-screen">
        ...
        <div className="grid grid-cols-4 gap-4">
            ...
            <div className="sticky top-4 bg-white p-4 rounded-lg shadow">
                ...
                <li>
                    <Button className={`justify-between w-full text-left ${selectedItem === 'tv' ? 'bg-blue-500 text-white' : ''}`} onClick={() => handleCategory('tv')}>
                        <span>Séries</span>
                        <Badge>{amountSeries}</Badge>
                    </Button>
                </li>
                <li>
                    <Button className={`justify-between w-full text-left ${selectedItem === 'movie' ? 'bg-blue-500 text-white' : ''}`} onClick={() => handleCategory('movie')}>
                    <span>Filmes</span>
                    <Badge>{amountMovies}</Badge>
                    </Button>
                </li>
                <li>
                    <Button className={`justify-between w-full text-left ${selectedItem === 'collection' ? 'bg-blue-500 text-white' : ''}`} onClick={() => handleCategory('collection')}>
                    <span>Coleções</span>
                    <Badge>{amountCollections}</Badge>
                    </Button>
                </li>
                <li>
                    <Button className={`justify-between w-full text-left ${selectedItem === 'person' ? 'bg-blue-500 text-white' : ''}`} onClick={() => handleCategory('person')}>
                    <span>Pessoas</span>
